...
do_sign(uint8_t *sigbuf, uint8_t *unused, size_t *sigbuflen,
	...
	do {
        ...
        v = PQCLEAN_FALCONPADDED1024_BH_AARCH64_comp_encode(tmp_sig, *sigbuflen, r.sig);
...
do_verify(
    ...
    v = PQCLEAN_FALCONPADDED1024_BH_AARCH64_comp_decode(sig, sig_falcon_body, sig_falcon_body_len);
    if (v == 0) {
        return -1;
    }
    ...
    */
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    PQCLEAN_FALCONPADDED1024_BH_AARCH64_hash_to_point_ct(&sc, (uint16_t *) recon_hm, FALCON_LOGN, tmp.b);
    inner_shake256_ctx_release(&sc);
...
PQCLEAN_FALCONPADDED1024_BH_AARCH64_crypto_sign(
    ...
    if (do_sign(sm + 1, sigbuf, &sigbuflen, m, mlen, sk,
        //
        ctx_classical, signature_len_classical) < 0) {
        return -1;
    }
...
PQCLEAN_FALCONPADDED1024_BH_AARCH64_crypto_sign_open(
    ...
    if (do_verify(sm + 1, sigbuf, sigbuflen,
                  sm + PQCLEAN_FALCONPADDED1024_BH_AARCH64_CRYPTO_BYTES, pmlen, pk, 
                  // 
                  ctx_classical) < 0) {
        return -1;
    }
