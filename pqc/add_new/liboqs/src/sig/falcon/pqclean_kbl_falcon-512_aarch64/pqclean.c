...
/* see api.h */
int
PQCLEAN_KBL_FALCON512_AARCH64_crypto_sign_keypair(
    ...
    PQCLEAN_KBL_FALCON512_AARCH64_keygen(&rng, f, g, F, NULL, h, FALCON_LOGN, tmp.b);
    ...
    u = 1;
    v = PQCLEAN_KBL_FALCON512_AARCH64_trim_i8_encode(
            sk + u, PQCLEAN_KBL_FALCON512_AARCH64_CRYPTO_SECRETKEYBYTES - u,
            f, PQCLEAN_KBL_FALCON512_AARCH64_max_fg_bits[FALCON_LOGN]);
    if (v == 0) {
        return -1;
    }
    u += v;
    v = PQCLEAN_KBL_FALCON512_AARCH64_trim_i8_encode(
            sk + u, PQCLEAN_KBL_FALCON512_AARCH64_CRYPTO_SECRETKEYBYTES - u,
            g, PQCLEAN_KBL_FALCON512_AARCH64_max_fg_bits[FALCON_LOGN]);
    if (v == 0) {
        return -1;
    }
    u += v;
    ...
    u += v;
    if (u != PQCLEAN_KBL_FALCON512_AARCH64_CRYPTO_SECRETKEYBYTES) {
        return -1;
    }
    ...
    v = PQCLEAN_KBL_FALCON512_AARCH64_modq_encode(
            pk + 1, PQCLEAN_KBL_FALCON512_AARCH64_CRYPTO_PUBLICKEYBYTES - 1,
            h, FALCON_LOGN);
    if (v != PQCLEAN_KBL_FALCON512_AARCH64_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
    ...
    u = 1;
    v = PQCLEAN_KBL_FALCON512_AARCH64_trim_i8_decode(
            f, PQCLEAN_KBL_FALCON512_AARCH64_max_fg_bits[FALCON_LOGN],
            sk + u, PQCLEAN_KBL_FALCON512_AARCH64_CRYPTO_SECRETKEYBYTES - u);
    if (v == 0) {
        return -1;
    ...
    if (!PQCLEAN_KBL_FALCON512_AARCH64_complete_private(G, f, g, F, tmp.b)) {
        return -1;
    }
    ...
    PQCLEAN_KBL_FALCON512_AARCH64_hash_to_point_ct(&sc, r.hm, FALCON_LOGN, tmp.b);
     ...
     */
    PQCLEAN_KBL_FALCON512_AARCH64_sign_dyn(r.sig, &sc, f, g, F, G, r.hm, tmp.b);
    
    // (Mizzou 2025) revised
    v = PQCLEAN_KBL_FALCON512_AARCH64_comp_encode(sigbuf - size_r_ecdsa, *sigbuflen, r.sig);
    if (v != 0) {
        inner_shake256_ctx_release(&sc);
        *sigbuflen = v;
        return 0;
...
do_verify(
    ...
    if (PQCLEAN_KBL_FALCON512_AARCH64_modq_decode( (uint16_t *) h,
            pk + 1, PQCLEAN_KBL_FALCON512_AARCH64_CRYPTO_PUBLICKEYBYTES - 1, FALCON_LOGN)
            != PQCLEAN_KBL_FALCON512_AARCH64_CRYPTO_PUBLICKEYBYTES - 1) {
        return -1;
    }
    ...
    v = PQCLEAN_KBL_FALCON512_AARCH64_comp_decode(sig, sigbuf, sigbuflen);
    if (v == 0) {
        return -1;
    }
    if (v != sigbuflen) {
    ...
    inner_shake256_inject(&sc, m, mlen);
    inner_shake256_flip(&sc);
    PQCLEAN_KBL_FALCON512_AARCH64_hash_to_point_ct(&sc, (uint16_t *) hm, FALCON_LOGN, tmp.b);
    inner_shake256_ctx_release(&sc);

    /*
     * Verify signature.
     */
    if (!PQCLEAN_KBL_FALCON512_AARCH64_verify_raw(hm, sig, h, (int16_t *) tmp.b)) {
        return -1;
    }
    return 0;
}
...
/* see api.h */
int
PQCLEAN_KBL_FALCON512_AARCH64_crypto_sign_signature(
    uint8_t *sig, size_t *siglen,
    const uint8_t *m, size_t mlen, const uint8_t *sk) {
    size_t vlen;
    
    vlen = PQCLEAN_KBL_FALCON512_AARCH64_CRYPTO_BYTES - NONCELEN - 1;
...
/* see api.h */
int
PQCLEAN_KBL_FALCON512_AARCH64_crypto_sign_verify(
...
/* see api.h */
int
PQCLEAN_KBL_FALCON512_AARCH64_crypto_sign(
    ...
    pm = sm + 2 + NONCELEN;
    sigbuf = pm + 1 + mlen;
    sigbuflen = PQCLEAN_KBL_FALCON512_AARCH64_CRYPTO_BYTES - NONCELEN - 3;
...
/* see api.h */
int
PQCLEAN_KBL_FALCON512_AARCH64_crypto_sign_open(
