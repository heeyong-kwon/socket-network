...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t *sigbuflen,
    ...
    // (Mizzou, 2025) revised
    v = PQCLEAN_FALCON1024_BH_AARCH64_comp_encode(sigbuf - NONCELEN, *sigbuflen, r.sig);
    // Original code copied from liboqs
    // v = PQCLEAN_FALCON1024_BH_AARCH64_comp_encode(sigbuf, *sigbuflen, r.sig);
    if (v != 0) {
        inner_shake256_ctx_release(&sc);
        *sigbuflen = v;
        return 0;
...
do_verify(
    ...
    v = PQCLEAN_FALCON1024_BH_AARCH64_comp_decode(sig, sigbuf, sigbuflen);
    if (v == 0) {
        return -1;
    }
    if (v != sigbuflen) {
